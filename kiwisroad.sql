...
ALTER TYPE HITO_T ADD MEMBER FUNCTION obtenerActividades RETURN REF_ACTIVIDADES_T,
-- funcion que permite la agregacion formadaPor y devuelve las rutas a las que pertenece el hito
ALTER TYPE HITO_T ADD MEMBER FUNCTION formaParteDe RETURN REF_RUTAS_T
...
-- 1) Funcion que calcula la distacia a recorrer en una ruta basandose en la suma de las distancias
-- de todas las vias que conforman la ruta
...
CREATE OR REPLACE TYPE BODY RUTA_T AS MEMBER PROCEDURE listarActividadesDeRuta IS 
  act VARCHAR(50);
  BEGIN
